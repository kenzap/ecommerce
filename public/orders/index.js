(function (kCloud) {
    ...
    const timeConverterAgo = (now, time) => {
    ...
    const _this = {
        ...
        initListeners: () => {
            ...
            // view order
            kCloud.onClick('.view-order', _this.listeners.viewOrder);

            // remove order
            kCloud.onClick('.remove-order', _this.listeners.removeOrder);

            // table status change listener
            kCloud.onClick('.st-table li a', _this.listeners.changeStatus);

            // break here if initListeners is called more than once
            if(!_this.state.firstLoad) return;

            // modal success button
            kCloud.onClick('.modal .btn-primary', _this.listeners.modalSuccessBtn);
            
            // search orders
            kCloud.onKeyUp('.search-input', _this.listeners.searchOrders);
            // add product modal
        ...
        listeners: {
            ...
            <div class="st-modal st-opts mb-3 me-3 dropdown">
                ...
                kCloud.onClick('.st-modal li a', (e) => {
